...
int remplir_les_cases(int i,  int l, int c ,int random_p,int random_s){
    ...
    if(i==30){
        ...
        if(random_p%3 == 0){
            if(random_s%4 == 0){
                if ((l == 1 && (c == 0 || c == 2)) || l == 2){  // pieces en T dans tt les sens
                    return 0;
                }
                else{
                    return 1;
...
void zebi() {
    ...
    for (int i = 0; i < 49; i++) {
        ...
        for (int l = 0; l < 3 ; l++) {
            for (int c = 0; c < 3; c++) {
                matrix[i][l][c] = remplir_les_cases(i,l,c,random_p,random_s); //call remplir les cases
            }
        }
    }
